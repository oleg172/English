...
public class LearningWordSelectionView {
    ...
    public SendMessage words(Long chatId, Topic topic, Page<UserWord> result, Set<Long> selectedWordIds) {
        return telegramMessage.withKeyboard(chatId,
                "🎓 Выберите слова для обучения\n" +
                        "Топик: " + topic.getName(),
                keyboard.words(result.getContent(), selectedWordIds, result.getNumber(), result.getTotalPages())
        );
    }

    public EditMessageText editWords(Long chatId, Integer messageId, Topic topic, Page<UserWord> result, Set<Long> selectedWordIds) {
    ...
    public EditMessageText editNotEnoughSelectedWords(Long chatId, Integer messageId) {
        return telegramMessage.edit(chatId, messageId, "Нужно выбрать минимум 4 слова.");
    }
}
